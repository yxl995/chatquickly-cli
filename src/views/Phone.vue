<template>
  <div class="panel">
    <div style="font-size: 30px;font-weight: bold;color: #dddddd;margin: 150px 0">{{ $route.params.to }}</div>
    <div v-if="$route.params.isCall==1||isConnected" style="display: flex;flex-direction: column;align-items: center">
      <div style="color: #dddddd;font-size: 18px;margin-bottom: 150px">
    ...
    <div v-else style="display: flex;margin-top: 150px">
    ...
    <audio :srcObject="localStream" v-if="localRendered" muted autoplay></audio>
    <audio :srcObject="remoteStream" v-if="remoteRendered" autoplay></audio>
  </div>
</template>
  ...
  activated() {
    ...
    request({
      ...
      if (flag && isCall == 1) {
        ...
        socket.emit('call', {to, from: username, streamID: userName, type: 'audio'})
      } else if (!flag) {
